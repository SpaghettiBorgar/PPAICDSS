...
def mnist_resnet_transform():
    return transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Lambda(lambda x: x.repeat(MNIST_CHANNELS, 1, 1)),
            transforms.Normalize(
                (MNIST_MEAN,) * MNIST_CHANNELS,
                (MNIST_STD,) * MNIST_CHANNELS,
            ),
        ]
    )
...
def main():
    ...
    for _ in range(args.n_runs):
        ...
        model = torchvision.models.resnet18(num_classes=10,pretrained=False).to(device)
        # model = torchvision.models.densenet121(num_classes=10,pretrained=False).to(device)
        model = ModuleValidator.fix(model)
        make_resnet_dp_safe(model)
